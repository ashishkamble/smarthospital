...
//**********************************************************************************************
	if (empty($Cid)) 
	{
		...
		{
			$_SESSION[ 'alert' ] = "<img src='image/al.png' style='margin-right: 20px' width='30'>No record found. ";
			$Cid= "";
			$_SESSION[ 'message' ] ="";
		}
	}
...
//**********************************************************************************************
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		if(isset($_POST['login']) && !empty($_SESSION[ 'card' ]) && !empty($Cid)) 
		{
			...
			$history = $conn->escape_string($_POST['history']);
			...
			{  
				...
				//$sql = "UPDATE `users` SET `id`=1,`username`=\"ashish\",`SerialNumber`=12,`gender`=\"male\",`CardID`=12345,`history`=\"hello\" WHERE 1";
				$sqll = "UPDATE users SET username='$Uname',SerialNumber='$Number' ,gender='$gender',history = '$history' WHERE CardID='$CardID'";
...
<body>
  <header >
    <div class="head">
      ...
      <h1>SMART Hospital<br>
		Record System</h1>
    </div>
    <a href="view.php">Patients Logs</a>
  </header>
	...
	<table>
      ...
      <input type="radio" name="gender" value="Male" required /><label>Male</label ></td>
		...
		<tr>
			...
			<td><input type="text" placeholder="medical history" name="history" required /></td>
		</tr>
		<tr>
